...
namespace js
{
  ...
  namespace sax {
    ...
    //
    // This uses the nlohmann/json library for parsing the json string
    static Value parseJSON (std::string const& str)
    {
        using json = nlohmann::json;

        sax::sax_event_consumer sec;
    ...
    //
    // This uses the nlohmann/json library for parsing the cbor bytes
    static Value parseCBOR (std::vector<u_int8_t> bytes)
    {
        using json = nlohmann::json;

        sax::sax_event_consumer sec;

        if (!json::sax_parse(bytes, &sec, json::input_format_t::cbor))
            throw std::runtime_error("Failed to parse cbor bytes.");
