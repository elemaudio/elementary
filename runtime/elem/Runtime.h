...
public:
    ...
    void process(const FloatType** inputChannelData,
...
Runtime<FloatType>::Runtime(double sampleRate, int blockSize)
    ...
    , blockSize(blockSize)
{
    DefaultNodeTypes<FloatType>::forEach([this](std::string const& type, NodeFactoryFn&& fn) { registerNodeType(type, std::move(fn)); });
}
...
template <typename FloatType>
void Runtime<FloatType>::process(const FloatType** inputChannelData,
                                 size_t numInputChannels,
                                 FloatType** outputChannelData,
                                 size_t numOutputChannels,
                                 size_t numSamples,
                                 void* userData)
{
    if (rseqQueue.size() > 0)
    {
        ...
        rtRenderSeq = rseq;
        ELEM_DBG("[Native] New rtRenderSeq");
    }
...
{
    return sharedResourceMap.insert(name, std::make_shared<typename SharedResourceBuffer<FloatType>::element_type>(data, data + size));
}
...
std::shared_ptr<GraphRenderSequence<FloatType>> Runtime<FloatType>::buildRenderSequence()
{
    ...
    for (auto& ptr : sortedRoots)
    {
        ...
        std::for_each(visitOrder.begin(), visitOrder.end(),
                      [&](NodeId const& nid)
                      {
