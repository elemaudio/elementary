 ...
 */
export function tau2pole(t: NodeRepr_t | number): NodeRepr_t | number {
 ...
 */
export function db2gain(db: NodeRepr_t | number): NodeRepr_t | number {
 ...
 */
export function gain2db(gain: NodeRepr_t | number): NodeRepr_t | number {
  return select(
    el.ge(gain, 0),
 ...
 */
export function select(g: NodeRepr_t | number, a: NodeRepr_t | number, b: NodeRepr_t | number): NodeRepr_t | number {
  return el.add(
    el.mul(g, a),
    el.mul(el.sub(1, g), b),
  );
 ...
 * @returns {core.Node}
 */
export function hann(t: NodeRepr_t | number): NodeRepr_t | number {
